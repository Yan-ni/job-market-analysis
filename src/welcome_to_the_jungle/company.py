...
class Company:
    ...
    def exists_in_db(self) -> bool:
        self.__db_cur.execute(
            """Select 1 FROM companies WHERE id=%(id)s""", {"id": self.get_id()}
        )
    ...
    def load_from_db(self) -> None:
        ...
        self.__db_cur.execute(
            """SELECT * FROM companies WHERE id=%(id)s""", {"id": self.get_id()}
        )
    ...
    def __scrape_office_location(self) -> str:
        office_location_tag = self.__soup.select_one(
            'div[data-testid="showcase-header-office"]'
        )

        return (
            office_location_tag.get_text() if office_location_tag is not None else None
        )
    ...
    def __scrape_website_url(self) -> str:
        website_url_tag = self.__soup.select_one(
            'div[data-testid="showcase-header-website"] a'
        )

        return (
            website_url_tag.attrs.get("href") if website_url_tag is not None else None
        )
    ...
    def __scrape_presentation(self) -> str:
        ...
        return (
            Scraper.get_soup_text(text_block_tags[0])
            if len(text_block_tags) >= 1
            else None
        )
    ...
    def __scrape_looking_for(self) -> str:
        ...
        return (
            Scraper.get_soup_text(text_block_tags[1])
            if len(text_block_tags) >= 2
            else None
        )
    ...
    def __scrape_good_to_know(self) -> str:
        ...
        return (
            Scraper.get_soup_text(text_block_tags[2])
            if len(text_block_tags) >= 3
            else None
        )
    ...
    def __scrape_creation_year(self) -> str:
        creation_year_tag = self.__soup.select_one(
            'span[data-testid="stats-creation-year"]'
        )
    ...
    def __scrape_number_employees(self) -> str:
        number_employees_tag = self.__soup.select_one(
            'span[data-testid="stats-nb-employees"]'
        )

        return (
            number_employees_tag.get_text()
            if number_employees_tag is not None
            else None
        )
    ...
    def __scrape_parity_percent_women(self) -> str:
        parity_percent_women_tag = self.__soup.select_one(
            'span[data-testid="stats-parity-women"]'
        )

        return (
            parity_percent_women_tag.get_text()
            if parity_percent_women_tag is not None
            else None
        )
    ...
    def __scrape_parity_percent_men(self) -> str:
        parity_percent_men_tag = self.__soup.select_one(
            'span[data-testid="stats-parity-men"]'
        )

        return (
            parity_percent_men_tag.get_text()
            if parity_percent_men_tag is not None
            else None
        )
    ...
    def __scrape_average_age(self) -> str:
        average_age_tag = self.__soup.select_one(
            'span[data-testid="stats-average-age"]'
        )
