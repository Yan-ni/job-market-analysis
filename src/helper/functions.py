    ...
    try:
        ...
        if not job_offer.exists_in_db():
            if not job_offer_company.exists_in_db():
                ...
                logging.debug(
                    f'saving the "company":"{job_offer_company.get_name()}" data in the database.'
                )

            job_offer.scrape_all_attributes()
            logging.debug(
                f'saving the "job offer":"{job_offer.get_title()}" data in the database.'
            )
            job_offer.save_to_db()
        else:
            logging.debug(
                f'"job offer":"({job_offer.get_id()}, {job_offer_company.get_id()})" already exists!'
            )
...
def scrape_jobs(args):
    ...
    logging.info(
        f"retrieving job offers list from search page: {{query: {search_page.get_query()}, location: {search_page.get_location()}}}"
    )
